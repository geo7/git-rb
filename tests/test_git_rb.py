...
def test_git_rb_successful_rebase(mock_prompt_ask, git_repo: Path, run_git_rb):
    ...
    assert not stderr.replace("Successfully rebased and updated refs/heads/main.", "").strip() # Only rebase success message
    ...
    rebased_log = subprocess.run(
        ["git", "log", "--pretty=format:%s"],
        cwd=git_repo, capture_output=True, text=True, check=True
    ).stdout.strip().splitlines()
    assert "fix: Update file1" == rebased_log[0]
    assert "feat: Add second file" == rebased_log[1]
    assert "feat: Initial commit" == rebased_log[2]
